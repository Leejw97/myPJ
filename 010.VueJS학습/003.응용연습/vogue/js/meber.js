...
$(()=>{ // jQB
    ...
    //
    // focus의 반대 -> blur(블러)
    // 대상: input[type=text][id!=email2][class!=serach],input[type=password]
    // 요소[속성][속성] -> 한 요소의 여러 속성을 설정할 수 있다.
    // 참고) != 선택자는 제이쿼리 전용이다(CSS에서는 사용불가)
    $("input[type=text][id!=email2][class!=serach],input[type=password]")
    .blur(function(){
    ...
    ********************************************/
    ...
    $("#btnj").click(function(e){
        ...
        // 대상: input[type=text][id!=email2][class!=serach],input[type=password]
        // 이벤트발생 메소드: trigger(이벤트명)
        $("input[type=text][id!=email2][class!=serach],input[type=password]").trigger("blur");
